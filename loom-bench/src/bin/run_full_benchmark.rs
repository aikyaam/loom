...
use loom_core::{
    decode_track, decode_track_partial, encode_session, encode_track_with_config, EncoderConfig,
};
...
fn main() {
    ...
    println!("================================================================================");
    println!(
        "Test Signal Duration : {} seconds ({} samples, {} bytes raw PCM per track)",
        seconds, length, raw_bytes
    );
    ...
    let corpora = vec![
        ...
        (
            "Drum Transients",
            generate_drum_transient(length, sample_rate),
        ),
    ...
    println!(
        "{:<18} | {:<8} | {:<12} | {:<8} | {:<14} | {:<14}",
        "Signal Type", "Level", "Comp Bytes", "Ratio %", "Enc Throughput", "Dec Throughput"
    );
    println!("--------------------------------------------------------------------------------------------------");
    ...
    for (sig_name, signal) in &corpora {
        ...
        for &level in &[0u8, 2u8, 5u8, 8u8] {
            ...
            println!(
                "{:<18} | {:<8} | {:<12} | {:<7.2}% | {:<11.2} MB/s | {:<11.2} MB/s",
                sig_name,
                format!("Loom-{}", level),
                compressed.len(),
                ratio,
    ...
    println!(
        "{:<18} | {:<12} | {:<8} | {:<14}",
        "Codec / Level", "Comp Bytes", "Ratio %", "Enc Throughput"
    );
    println!("----------------------------------------------------------------------------------");
    ...
    let indep_flac_bytes: usize = stems
        .iter()
        .map(|s| {
    ...
    let session_loom_bytes =
        encode_session(&stems, &stem_names, sample_rate, 16, 4096, None, None, None)
            .unwrap()
            .len();
    ...
    let savings =
        ((indep_flac_bytes as f64 - session_loom_bytes as f64) / indep_flac_bytes as f64) * 100.0;

    println!(
        "Total Raw PCM Size     : {} bytes ({:.2} MB)",
        total_raw_bytes,
        total_raw_bytes as f64 / (1024.0 * 1024.0)
    );
    println!(
        "8 Separate FLAC Files  : {} bytes ({:.2}% ratio)",
        indep_flac_bytes, flac_multitrack_ratio
    );
    println!(
        "1 Loom Session (.loom) : {} bytes ({:.2}% ratio)",
        session_loom_bytes, loom_session_ratio
    );
    ...
    println!(
        "Range Extraction (1 sec slice from 5 sec track) Latency: {:.3} us",
        seek_lat.as_nanos() as f64 / 1000.0
    );
    println!("================================================================================\n");
}
